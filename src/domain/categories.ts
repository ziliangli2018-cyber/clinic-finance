...
const RULES: readonly CategoryRule[] = [
  { category: 'Health fund receipts', sign: 'positive', pattern: /\b(HICAPS|HEALTH FUND|INSURANCE SETTLEMENT)\b/i },
  { category: 'Patient receipts', sign: 'positive', pattern: /\b(PATIENT|TYRO SETTLEMENT|EFTPOS SETTLEMENT)\b/i },
  { category: 'Interest income', sign: 'positive', pattern: /\bINTEREST (CREDIT|PAID|EARNED)\b/i },
  { category: 'Wages & super', sign: 'negative', pattern: /\b(PAYROLL|SUPERANNUATION|SUPER PAYMENT)\b/i },
  ...
  { category: 'Dental supplies', sign: 'negative', pattern: /\b(DENTAL SUPPLIES|DENTAL CONSUMABLES|CLINICAL SUPPLIES)\b/i },
  { category: 'Laboratory fees', sign: 'negative', pattern: /\b(DENTAL LAB|LABORATORY|CROWN LAB)\b/i },
  { category: 'Equipment & maintenance', sign: 'negative', pattern: /\b(EQUIPMENT|AUTOCLAVE|CHAIR SERVICE)\b/i },
  { category: 'Utilities', sign: 'negative', pattern: /\b(ELECTRICITY|ENERGY|WATER|INTERNET|BROADBAND)\b/i },
  { category: 'Software & subscriptions', sign: 'negative', pattern: /\b(SOFTWARE|SUBSCRIPTION|PRACTICE CLOUD)\b/i },
  { category: 'Insurance', sign: 'negative', pattern: /\b(INSURANCE PREMIUM|PROFESSIONAL INDEMNITY)\b/i },
  { category: 'Merchant & bank fees', sign: 'negative', pattern: /\b(MERCHANT FEE|BANK FEE|PROCESSING FEE)\b/i },
  ...
  { category: 'Personal spending', sign: 'negative', pattern: /\b(PERSONAL SHOPPING|HOMEWARES)\b/i },
];

/** Manual choices survive every processing pass, including a manual null choice. */
export function categoriseTransaction(transaction: Transaction, categories: readonly Category[]): Transaction {
  ...
  if (transaction.transferPairId) {
    ...
    if (transfer) return { ...transaction, categoryId: transfer.id, categorySource: 'rule', receiptStatus: 'not_required' };
  }
