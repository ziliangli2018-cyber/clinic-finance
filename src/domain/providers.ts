...
export class MockFinancialProvider implements FinancialDataProvider {
  ...
  async connect() {
    return (await this.loadDataset()).connections;
  }
  ...
  async getAccounts(connectionId: string) {
    return (await this.connection(connectionId)).accounts.filter(
      (account) => account.connectionId === connectionId,
    );
  }
  async getBalances(connectionId: string) {
    return (await this.getAccounts(connectionId)).map((account) => ({
  ...
  async getTransactions(connectionId: string, since?: string) {
    ...
    return data.transactions.filter(
      (transaction) =>
        accounts.has(transaction.accountId) && (!since || transaction.postedAt >= since),
    );
  }
