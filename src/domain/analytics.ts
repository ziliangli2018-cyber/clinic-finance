...
export function filterAccounts(dataset: FinanceDataset, scope: FinanceScope = {}): Account[] {
  ...
  const explicitAccount =
    scope.accountId && scope.accountId !== 'all' ? scope.accountId : undefined;
  const entities = new Map(
    dataset.entities
      .filter((entity) => entity.organisationId === dataset.organisation.id)
  ...
  return dataset.accounts.filter((account) => {
    ...
    return (
      ...
      scope.includePersonal === true ||
      Boolean(explicitEntity || explicitAccount)
    );
  });
}
...
export function calculateSummary(
  ...
  const expenseCents = posted.reduce(
    (total, transaction) => total + Math.max(-transaction.amountCents, 0),
    0,
  );
  return {
    balanceCents: filterAccounts(dataset, scope).reduce(
    ...
    missingReceiptCount: posted.filter(
      (transaction) => transaction.amountCents < 0 && transaction.receiptStatus === 'missing',
    ).length,
...
export function calculateDailyCashflow(
  ...
  if (dateValue(to) - dateValue(from) > 3660 * 86_400_000)
    throw new Error('Cash-flow chart range cannot exceed ten years.');
