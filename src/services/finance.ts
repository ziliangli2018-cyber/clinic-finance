...
async function readTable(table: string, organisationId?: string): Promise<Row[]> {
  ...
  for (let offset = 0; ; offset += 1000) {
    let query = supabase.from(table).select('*').order('id').range(offset, offset + 999);
...
export async function loadDataset(organisation: Organisation): Promise<FinanceDataset> {
  const [entities, connections, accounts, categories, transactions] = await Promise.all([
    readTable('entities', organisation.id), readTable('bank_connections', organisation.id),
    readTable('accounts', organisation.id), readTable('categories'), readTable('transactions', organisation.id),
  ]);
  return { organisation, entities: entities.map(camel), connections: connections.map(camel), accounts: accounts.map(camel), categories: categories.map(camel), transactions: transactions.map(camel) } as unknown as FinanceDataset;
}
...
export async function syncBank(organisationId: string) {
  ...
  if (error) throw new Error(`Bank sync failed. Check that local Edge Functions are running and mock data is enabled. ${error.message}`);
}
export async function saveCategory(transactionId: string, categoryId: string) {
  if (!supabase) throw new Error('Supabase is not configured');
  const { error } = await supabase.rpc('set_transaction_category', { transaction_id: transactionId, category_id: categoryId });
  if (error) throw error;
}
