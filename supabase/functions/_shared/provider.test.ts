...
Deno.test('mock integration requires all explicit server gates', () => {
  for (const [env, allow, demo] of [
    ['production', 'true', true], [undefined, 'true', true], ['demo', undefined, true],
    ['demo', 'false', true], ['development', 'true', false], ['staging', 'true', true],
  ] as const) {
    let rejected = false;
    try { assertMockAllowed(env, allow, demo); } catch { rejected = true; }
    if (!rejected) throw new Error(`Mock gate accepted invalid combination ${env}/${allow}/${demo}`);
  }
...
Deno.test('stable identifiers are deterministic and isolated by tenant', async () => {
  ...
  if (!/^[\da-f]{8}-[\da-f]{4}-5[\da-f]{3}-[89ab][\da-f]{3}-[\da-f]{12}$/.test(first)) throw new Error('Invalid UUID');
});

Deno.test('live provider remains explicitly unimplemented', () => {
  let rejected = false;
  try { bankProvider('basiq', 'production', 'false', false); } catch (error) {
