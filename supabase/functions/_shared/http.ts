...
export function headersFor(request: Request): Headers {
  ...
  const allowed = (
    Deno.env.get('CORS_ALLOWED_ORIGINS') ?? 'http://localhost:5173,http://127.0.0.1:5173'
  )
    .split(',')
...
export function failure(request: Request, error: unknown): Response {
  ...
  return response(request, 500, {
    error: 'Request failed. Try again or contact your administrator.',
  });
}
...
export async function organisationIdFrom(request: Request): Promise<string> {
  ...
  if (
    ...
    !/^[\da-f]{8}-[\da-f]{4}-[\da-f]{4}-[\da-f]{4}-[\da-f]{12}$/i.test(body.organisationId)
  ) {
