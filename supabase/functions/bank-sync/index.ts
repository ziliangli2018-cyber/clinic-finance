...
export async function handler(request: Request): Promise<Response> {
  ...
  try {
    ...
    const { data: organisation, error } = await context.admin.from('organisations').select('id,is_demo').eq('id', organisationId).single();
    if (error || !organisation) throw new HttpError(403, 'Organisation unavailable');
    const { data: connections, error: connectionError } = await context.admin.from('bank_connections').select('provider')
      .eq('organisation_id', organisationId);
    if (connectionError) throw new HttpError(503, 'Unable to read bank connections');
    if (connections?.some((row) => row.provider !== 'mock')) throw new HttpError(501, 'Live bank sync is not available in version 0.1');
    const provider = bankProvider('mock', Deno.env.get('APP_ENV'), Deno.env.get('ALLOW_MOCK_DATA'), organisation.is_demo);
    ...
    const { data, error: ingestError } = await context.admin.rpc('ingest_mock_dataset', {
      organisation_id: organisationId, actor_user_id: context.user.id, dataset,
    });
    if (ingestError) {
      if (ingestError.code === '42501') throw new HttpError(403, 'Demo sync disabled or access revoked');
