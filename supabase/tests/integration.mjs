...
assert.match(
  ...
  'Integration fixtures are permitted only on local Supabase',
);
...
await callOk(
  'owner sign up',
  owner.auth.signUp({ email: `owner-${suffix}@example.invalid`, password }),
);
await callOk(
  'other sign up',
  other.auth.signUp({ email: `other-${suffix}@example.invalid`, password }),
);
const orgId = await callOk(
  'owner demo onboarding',
  owner.rpc('create_demo_organisation', { name: 'Integration Test Clinic' }),
);
const repeatedOrgId = await callOk(
  'repeat onboarding',
  owner.rpc('create_demo_organisation', { name: 'Repeat' }),
);
assert.equal(orgId, repeatedOrgId);
await callOk(
  'other demo onboarding',
  other.rpc('create_demo_organisation', { name: 'Isolated Test Clinic' }),
);
...
const sync = await callOk(
  'initial bank sync',
  owner.functions.invoke('bank-sync', { body: { organisationId: orgId } }),
);
assert.equal(sync.syncedAccounts, 8);
assert.ok(sync.syncedTransactions > 100);
const transactions = await callOk(
  'read transactions',
  owner.from('transactions').select('*').eq('organisation_id', orgId),
);
...
await callOk(
  'manual category',
  owner.rpc('set_transaction_category', { transaction_id: target.id, category_id: category.id }),
);
await callOk(
  'repeat bank sync',
  owner.functions.invoke('bank-sync', { body: { organisationId: orgId } }),
);
const after = await callOk(
  'read after repeat',
  owner.from('transactions').select('*').eq('organisation_id', orgId),
);
assert.equal(after.length, transactions.length, 'Retry must not duplicate transactions');
assert.equal(
  after.find((row) => row.id === target.id).category_id,
  category.id,
  'Retry must preserve manual category',
);
assert.equal(after.find((row) => row.id === target.id).category_source, 'manual');
assert.deepEqual(
  await callOk(
    'cross-tenant read',
    other.from('transactions').select('id').eq('organisation_id', orgId),
  ),
  [],
);
const forbiddenCategory = await other.rpc('set_transaction_category', {
...
const forgedIngest = await owner.rpc('ingest_mock_dataset', {
...
console.log(
  `PASS: local Auth, idempotent onboarding, 8 accounts / ${transactions.length} transactions, sync retries, category preservation, cross-tenant authorization and future endpoint 501s.`,
);
