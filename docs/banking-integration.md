...
The `FinancialDataProvider` contract in `src/domain/providers.ts` exposes connect, refresh, accounts, balances, transactions and disconnect. `MockFinancialProvider` implements these operations over injected fictitious fixtures; `BasiqFinancialProvider` fails closed. The backend `BankProvider` orchestration adapter in `supabase/functions/_shared/provider.ts` uses the mock provider behind environment and ownership checks, then returns normalised data to a trusted ingestion boundary. It rekeys organisation-owned IDs per tenant. Categories retain their shared catalogue IDs. A retry updates existing records by provider identity and preserves manual categorisation; it does not duplicate transactions. Missing mock connections are created during first sync, so an empty demo organisation can onboard successfully.
