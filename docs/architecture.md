...
Clinic Finance is a static React application backed by Supabase. GitHub is the home for source code, reviews, issues, migrations, tests and releases. Pages hosts only the compiled browser application. Postgres becomes the source of truth when Supabase mode is enabled.

```mermaid
flowchart TD
  Repo[Private GitHub repository] --> CI[GitHub Actions validation and build]
...
Authentication uses Supabase in the browser. Auth site URLs and allowed redirects must match each deployed origin and base path. Provider credentials and authorisation codes for future banking integrations terminate at backend endpoints, never at an arbitrary Pages route.
