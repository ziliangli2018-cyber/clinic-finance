...
test('account links filter transactions and survive a Pages deep-link refresh', async ({ page }) => {
...
test('categorisation updates the session ledger and removes reviewed entries', async ({ page }) => {
  ...
  await dialog.getByRole('combobox', { name: 'Category', exact: true }).selectOption({ label: 'Miscellaneous business expenses' });
