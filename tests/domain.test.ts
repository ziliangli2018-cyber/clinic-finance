...
import { calculateCategoryBreakdown, calculateDailyCashflow, calculateMonthlyCashflow, calculateSummary, filterTransactions } from '../src/domain/analytics.ts';
...
function entry(id: string, amountCents: number, overrides: Partial<Transaction> = {}): Transaction {
  return {
    id: stableId('test', id), organisationId: base.organisation.id, accountId: clinicA.id,
    providerTransactionId: id, postedAt: '2026-09-10', description: 'EFT PAYMENT', merchant: null,
    amountCents, currency: 'AUD', categoryId: null, categorySource: 'uncategorised',
    status: 'posted', transferPairId: null, receiptStatus: 'not_required', ...overrides,
  };
}

function dataset(transactions: Transaction[]): FinanceDataset {
  return { ...base, accounts: base.accounts.map((account) => ({ ...account, balanceCents: account.id === clinicA.id ? 100_000 : 0 })), transactions };
}

function transferLegs(): Transaction[] {
  return [
    entry('from', -20_000, { description: 'INTERNAL TRANSFER TO OWN ACCOUNT' }),
    entry('to', 20_000, { description: 'INTERNAL TRANSFER FROM OWN ACCOUNT', accountId: reserveA.id }),
  ];
}
...
describe('deterministic fictitious ledger', () => {
  it('covers exactly 90 days with 8 accounts, 2 clinics and personal, using consistent UUID relationships', () => {
    ...
    expect(base.accounts.filter(account => account.entityId === personal.entityId).map(account => account.kind)).toEqual(['transaction', 'credit_card']);
    for (const description of ['LOAN REPAYMENT', 'GST RESERVE', 'ATO TAX GST PAYMENT']) expect(base.transactions.some(transaction => transaction.description.includes(description))).toBe(true);
    ...
    for (const row of [base.organisation, ...base.entities, ...base.connections, ...base.accounts, ...base.categories, ...base.transactions]) expect(row.id).toMatch(uuid);
    expect(new Set(base.transactions.map((transaction) => transaction.id)).size).toBe(base.transactions.length);
    for (const transaction of base.transactions) {
      ...
      expect(transaction.categoryId === null || base.categories.some((category) => category.id === transaction.categoryId)).toBe(true);
    }
    for (const account of base.accounts) {
      const connection = base.connections.find((candidate) => candidate.id === account.connectionId)!;
...
describe('conservative internal transfer matching', () => {
  it('matches uniquely reciprocal owned entries across a weekend and is order-independent', () => {
    ...
    expect(detectInternalTransfers([...legs].reverse(), base.accounts)[0].transferPairId).toBe(result[0].transferPairId);
    expect(legs[0].transferPairId).toBeNull();
  });

  it('leaves every leg unmatched when an amount has multiple possible counterparts', () => {
    const legs = [...transferLegs(), entry('ambiguous', 20_000, { accountId: clinicB.id, description: 'INTERNAL TRANSFER' })];
    expect(detectInternalTransfers(legs, base.accounts).every((transaction) => transaction.transferPairId === null)).toBe(true);
  });

  it('requires uniqueness in both directions, even when only one side has alternatives', () => {
    const legs = [...transferLegs(), entry('ambiguous-debit', -20_000, { accountId: clinicB.id, description: 'INTERNAL TRANSFER' })];
    expect(detectInternalTransfers(legs, base.accounts).every((transaction) => transaction.transferPairId === null)).toBe(true);
  });

  it('never pairs equal-value unrelated income and expenditure', () => {
    const legs = transferLegs().map((transaction) => ({ ...transaction, description: transaction.amountCents > 0 ? 'PATIENT RECEIPTS' : 'DENTAL SUPPLIES' }));
    expect(detectInternalTransfers(legs, base.accounts).every((transaction) => transaction.transferPairId === null)).toBe(true);
  });
...
describe('cash-flow reporting', () => {
  it('excludes pending and matched internal transfers from cash flow while preserving them in the ledger', () => {
    const transactions = [
      entry('income', 10_001), entry('expense', -3_002, { receiptStatus: 'missing' }),
    ...
    expect(calculateSummary(data)).toMatchObject({
      incomeCents: 10_001, expenseCents: 3_002, netCashflowCents: 6_999,
      transactionCount: 2, pendingCount: 1, internalTransferCount: 2, missingReceiptCount: 1,
    });
  ...
  it('validates pairs before filtering, so a single-account view does not treat a reserve move as an expense', () => {
    ...
    expect(calculateSummary(data, { accountId: clinicA.id })).toMatchObject({
      incomeCents: 0, expenseCents: 0, netCashflowCents: 0, internalTransferCount: 1,
    });
  });
  ...
  it('does not hide cash flow behind incomplete or invalid persisted pair flags', () => {
    ...
    expect(calculateSummary(mismatched)).toMatchObject({ incomeCents: 600, expenseCents: 700, netCashflowCents: -100 });
  });

  it('excludes personal by default and includes it only through explicit scope', () => {
    const data = dataset([entry('clinic', 100), entry('personal', -45, { accountId: personal.id })]);
    expect(calculateSummary(data).expenseCents).toBe(0);
    expect(calculateSummary(data, { includePersonal: true }).expenseCents).toBe(45);
    expect(calculateSummary(data, { entityId: personal.entityId })).toMatchObject({ incomeCents: 0, expenseCents: 45 });
    expect(calculateSummary(data, { accountId: personal.id }).expenseCents).toBe(45);
    expect(filterTransactions(data, { entityId: clinicA.entityId, accountId: personal.id })).toEqual([]);
  });
  ...
  it('uses inclusive UTC date boundaries and retains current balance separately from period flow', () => {
    ...
    expect(calculateSummary(data, scope)).toMatchObject({ balanceCents: 100_000, incomeCents: 101, expenseCents: 20, netCashflowCents: 81 });
    const daily = calculateDailyCashflow(data, scope);
    expect(daily).toHaveLength(20);
    expect(daily[1]).toEqual({ date: '2026-09-02', incomeCents: 0, expenseCents: 0, netCashflowCents: 0 });
    expect(calculateMonthlyCashflow(data, scope)).toEqual([{ date: '2026-09', incomeCents: 101, expenseCents: 20, netCashflowCents: 81 }]);
  });
  ...
  it('keeps category totals and chart totals exactly reconciled to integer-cent summaries', () => {
    ...
    expect(calculateCategoryBreakdown(base, scope).reduce((sum, row) => sum + row.amountCents, 0)).toBe(summary.expenseCents);
    expect(daily.reduce((sum, row) => sum + row.incomeCents, 0)).toBe(summary.incomeCents);
    expect(daily.reduce((sum, row) => sum + row.netCashflowCents, 0)).toBe(summary.netCashflowCents);
...
describe('categorisation and provider safety', () => {
  it('preserves manual corrections through reprocessing', () => {
    const transaction = entry('manual', -8_500, { description: 'DENTAL SUPPLIES', categoryId: base.categories[4].id, categorySource: 'manual' });
    expect(categoriseTransaction(transaction, base.categories)).toEqual(transaction);
    expect(categoriseTransaction({ ...transaction, categoryId: null }, base.categories).categoryId).toBeNull();
  });
  ...
  it('uses sign-aware rules and leaves unrecognised entries for review', () => {
    ...
    expect(categoriseTransaction(entry('supply', -150, { description: 'DENTAL SUPPLIES' }), base.categories).categoryId).toBe(supplies.id);
    expect(categoriseTransaction(entry('refund', 150, { description: 'DENTAL SUPPLIES REFUND' }), base.categories).categoryId).toBeNull();
    expect(categoriseTransaction(entry('unknown', -150), base.categories).categorySource).toBe('uncategorised');
  });
  ...
  it('scopes provider operations to a connection and refuses disconnected reads', async () => {
    ...
    expect(transactions.every(transaction => accounts.some(account => account.id === transaction.accountId) && transaction.postedAt >= '2026-09-01')).toBe(true);
